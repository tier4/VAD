...
class MapHungarianAssigner3D(BaseAssigner):
    ...
    def assign(self,
        ...
        if torch.isnan(iou_cost).any() or torch.isinf(iou_cost).any():
            iou_cost = torch.clamp(iou_cost, min=-1e6, max=1e6)
            iou_cost = torch.where(torch.isnan(iou_cost), torch.ones_like(iou_cost) * 1e3, iou_cost)
            
        if torch.isnan(pts_cost).any() or torch.isinf(pts_cost).any():
            pts_cost = torch.clamp(pts_cost, min=-1e6, max=1e6)
            pts_cost = torch.where(torch.isnan(pts_cost), torch.ones_like(pts_cost) * 1e3, pts_cost)
